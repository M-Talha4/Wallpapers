...
import com.wallpaper.allwallpapers.DownloadActivity
import com.bumptech.glide.Glide
import com.wallpaper.allwallpapers.R
import java.util.concurrent.TimeUnit

class DownloadAdapter(val cc:Context, val list1:ArrayList<String>,val activity: Fragment): MaxAdViewAdListener, RecyclerView.Adapter<DownloadAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        Glide.with(cc).load(list1[position]).into(holder.image);
        holder.itemView.setOnClickListener {

            intent = Intent(cc, DownloadActivity::class.java)
            intent.putExtra("link1",list1[position])
